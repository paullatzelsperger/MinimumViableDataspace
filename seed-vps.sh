...
curl -si -X POST http://vps.beardyinc.com/aas/Browser/LocalFileOpen \
...
for url in 'http://vps.beardyinc.com/provider-manufacturing/cp' 'http://vps.beardyinc.com/provider-qna/cp'
do
  newman run \
    --folder "Seed" \
    --env-var "HOST=$url" \
...
newman run \
  ...
  --env-var "HOST=http://vps.beardyinc.com/provider-catalog-server/cp" \
...
curl --location "http://vps.beardyinc.com/consumer/cs/api/identity/v1alpha/participants/" \
...
curl --location "http://vps.beardyinc.com/provider/cs/api/identity/v1alpha/participants/" \
--header 'Content-Type: application/json' \
--header "x-api-key: $API_KEY" \
--data "$DATA_PROVIDER"
...
newman run \
  ...
  --env-var "ISSUER_ADMIN_URL=http://vps.beardyinc.com/issuer/ad" \
